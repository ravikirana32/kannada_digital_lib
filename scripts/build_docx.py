...
DATA=ROOT/"data/sarvajna/tripadis"; OUT=ROOT/"books/sarvajna/generated/docx/sarvajna-samagra.docx"
doc=Document()
doc.add_heading("ಸರ್ವಜ್ಞ ಸಮಗ್ರ ತ್ರಿಪದಿಗಳು",0)
doc.add_paragraph("ಮೂಲ ಕನ್ನಡ • ಭಾವಾರ್ಥ • ಜೀವನ ಸಂದೇಶ")
rows=[]
...
for d in sorted(rows,key=lambda x:x["tripadi_number"]):
    doc.add_heading(f"ತ್ರಿಪದಿ {d['tripadi_number']}",1)
    doc.add_paragraph(d["original"])
